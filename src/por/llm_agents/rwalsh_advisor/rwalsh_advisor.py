...
class RWalshAdvisorDeps(BaseModel):
    text_chunks: list[TextChunk]


class RWalshAdvisorOutput(BaseModel):
    phrase: StrictStr = Field(
        description=(
            "One short verbatim sentence or phrase copied exactly from the "
            "Rodolfo Walsh text chunks."
        ),
        min_length=1,
    )
...
agent = Agent(  # type: ignore
    ...
    deps_type=RWalshAdvisorDeps,
    output_type=NativeOutput(RWalshAdvisorOutput),
    retries=3,
)


@agent.system_prompt
async def get_system_prompt(ctx: RunContext[RWalshAdvisorDeps]) -> str:
    system_prompt = LLMAgent.read_file(
        file_path=str(Path(__file__).with_name("system-prompt.md"))
    )
...
class RWalshAdvisor(LLMAgent[RWalshAdvisorDeps, RWalshAdvisorOutput]):
